...
import { Project, Scenario, ScenarioStep } from "../types";
        ...
        <div className="bg-zinc-900/40 border border-zinc-800 rounded-2xl p-5 flex-1 flex flex-col gap-4">
          <div className="flex items-center justify-between">
            <h2 className="text-xs font-mono font-bold tracking-wider text-zinc-400 uppercase">
              Pipeline Steps Graph Execution Overview
            </h2>
            {selectedScenarioId && (
              <button 
                      ...
                      {(expandedStepId === step.id || isStepRunning) && (
                        <div className="flex flex-col gap-2 mt-2 pt-2 border-t border-zinc-900 font-mono text-[10px]">
                          <div>
                            ...
                            <pre className="bg-zinc-950 p-1.5 rounded text-zinc-300 text-[9.5px] truncate block max-h-16 overflow-y-auto">
                              {step.argumentsJson}
                            </pre>
                          </div>
            ...
            <div className="border-b border-zinc-800 pb-4 flex items-center justify-between">
              <div className="flex items-center gap-2">
                ...
                <div>
                  ...
                  <span className="text-[10.5px] font-mono text-zinc-500">
                    {selectedScenario ? `Orchestrating Flow Suite Target: ${selectedScenario.name}` : "Awaiting selection"}
                  </span>
                </div>
              </div>
              ...
              {selectedScenario && (
                <button
                  ...
                  className="px-4 py-1.5 font-mono font-bold text-xs rounded-xl bg-amber-500 text-zinc-950 hover:bg-amber-400 disabled:opacity-40 flex items-center gap-1.5 cursor-pointer transition-all"
                >
                  {isRunning ? (
                    <>
                            ...
                            {res.assertionResults.map((ass: any, assIdx: number) => (
                              <div key={assIdx} className="flex justify-between font-mono">
                                ...
                                <span className={ass.passed ? "text-emerald-400" : "text-red-400"}>
                                  {ass.passed ? "Passed" : `Failed (actual: ${ass.actualValue})`}
                                </span>
                              </div>
                            ))}
                          </div>
                  ...
                  <div className="border-t border-zinc-900 pt-3 mt-1 flex flex-col gap-2">
                    ...
                    <div className="grid grid-cols-2 gap-2 text-[10px]">
                      {Object.entries(runLogs.finalContext).map(([k, v]: any) => (
                        <div key={k} className="flex justify-between bg-zinc-900 border border-zinc-850 p-1.5 rounded">
              ...
              <div className="border-t border-zinc-800/80 pt-5 mt-4">
                <div className="bg-zinc-950/80 border border-zinc-800 rounded-xl p-4 flex flex-col gap-3.5">
                  <div className="flex items-center justify-between text-zinc-400 border-b border-zinc-900 pb-2">
                    ...
                    <span className="text-[10px] font-mono text-zinc-500">pipeline mapping suite</span>
                  </div>
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-3.5">
                    ...
                    <div>
                      <label className="text-[9.5px] font-mono text-zinc-500 block mb-1">Method Signature</label>
                      <select 
                        value={newStepMethod}
                        onChange={e => {
                    ...
                    <div className="flex flex-col gap-1.5">
                      ...
                      <div className="flex items-center gap-1.5">
                        <input 
                          ...
                          className="flex-1 bg-zinc-900 border border-zinc-800 rounded px-2 py-1 font-mono text-[10.5px] text-zinc-300"
                        />
                    ...
                    <div className="flex flex-col gap-1.5">
                      ...
                      <div className="flex items-center gap-1.5">
                        <input 
                          ...
                          className="flex-1 bg-zinc-900 border border-zinc-800 rounded px-1.5 py-1 font-mono text-[10.5px] text-zinc-300"
                        />
                        <select 
                          value={newStepAssOp} 
                          onChange={e => setNewStepAssOp(e.target.value as any)}
                          className="bg-zinc-900 border border-zinc-805 rounded px-1 py-1 font-mono text-[10px] text-zinc-400 cursor-pointer"
                        >
                  ...
                  <div className="flex justify-end gap-2 border-t border-zinc-900 pt-3">
                    <button 
                      ...
                      className="px-4 py-1.5 font-mono font-bold text-xs rounded-lg text-zinc-950 bg-amber-500 hover:bg-amber-400 disabled:opacity-40 flex items-center gap-1 cursor-pointer transition-all"
                    >
                      <Plus className="w-3.5 h-3.5" />
                      Append Step
                    </button>
